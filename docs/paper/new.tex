%% LunarAtlas: Draft 6 (Complete Revised Manuscript with New LTTB+Peaks Approach)
...
correctness of the pipeline.  The fraction of negative samples
before clamping ranges from 0.5\% (ID~3, high-quality plasma) to
76.8\% (ID~2, near-null acquisition), demonstrating that the
\textbf{Measurement~ID} is essential for identifying shot-to-shot
variability that would be hidden in bulk averaging.
LunarAtlas further provides a PDS4-aware PostgreSQL schema,
\textbf{adaptive LTTB-based downsampling with explicit peak-union
guarantees}, and a versioned API with sub-\SI{500}{\milli\second}
response times.  By establishing a ``single source of scientific
truth,'' the system offers a replicable methodology for
Chandrayaan-4, Artemis-era VIPER, and Commercial Lunar Payload
Services (CLPS) LIBS instruments.
\end{abstract}
\vspace{4pt}
\noindent\textbf{Keywords:} Laser-Induced Breakdown Spectroscopy;
Chandrayaan-3; Lunar Regolith; PDS4; Data Cleaning; Spectral
Reproducibility; Background Subtraction; NIST Validation;
Measurement~ID; LTTB Downsampling; Peak Preservation.
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item interactive visualisation preserving narrow emission-line
    peaks at all zoom levels.
\end{enumerate}
...
\begin{enumerate}[label=\textbf{(C\arabic*)},leftmargin=*,itemsep=3pt]
  ...
  \item \textbf{Measurement~ID semantics.}  A formal definition and
    ...
    shot, instrument configuration, and acquisition timestamp---with a
    full explanation of what it means, how it is derived, and why it
  ...
  \item \textbf{PDS4-aware data model.}  A normalised PostgreSQL
    ...
    wavelength $\rightarrow$ intensity), together with MD5 file
    checksums and algorithm-version metadata for every ingested file.

  \item \textbf{LTTB-based adaptive downsampling with peak-union
    guarantee.}  A mathematically rigorous formulation that combines
    Largest-Triangle-Three-Buckets (LTTB) channel-index-space
    decimation with explicit union of all local maxima, ensuring
    100\% retention of emission-line peaks at every zoom level, with
    dual zoom-saturation criteria preventing over-decimation.
...
including the critical Measurement~ID concept.
Section~\ref{sec:math} covers the new LTTB+peaks adaptive downsampling
formulation with dual saturation criteria.
Section~\ref{sec:api} describes the API and visualisation workflow.
Section~\ref{sec:results} presents cleaning results and validation.
...
\textbf{The LTTB+peaks strategy introduced in LunarAtlas specifically
addresses this gap by computing the union of LTTB-selected points with
all local maxima, guaranteeing 100\% peak retention regardless of bucket
size or zoom level.}
...
filtering, MD5 computation, \textbf{LTTB+peaks downsampling}, peak
...
Table~\ref{tab:schema} lists the core tables and their most important
columns.

\begin{table}[htbp]
\centering
\caption{Core LunarAtlas database tables.}
\label{tab:schema}
{\small
\begin{tabularx}{\columnwidth}{@{}lX@{}}
\toprule
...
\midrule
\texttt{mission} & id, name, agency, launch\_date \\
\addlinespace
\texttt{instrument} & id, mission\_id, name, type \\
\addlinespace
\texttt{observation} & id, instrument\_id, start/stop\_time, site \\
\addlinespace
\texttt{file\_version} & id, observation\_id, filename, md5,
  ingested\_at, algorithm\_version \\
\addlinespace
\texttt{measurement} & id, file\_version\_id, measurement\_id,
  time\_utc, laser\_energy\_v, n\_pulses \\
\addlinespace
\texttt{spectral\_data} & id, measurement\_id~(FK),
  wavelength\_nm, cleaned\_intensity \\
\addlinespace
\texttt{nist\_lines} & id, element, ionisation,
  wavelength\_nm, aki, ei/ek\_ev \\
\bottomrule
\end{tabularx}
}
\end{table}
...
For the archival file used in this study, the pipeline identifies
four valid background rows and four valid plasma rows, yielding four
background--plasma pairs.  Each pair is ordered by acquisition time:
the $k$-th plasma row is paired with the $k$-th background row.

The pipeline explicitly handles edge cases: if the number of
background and plasma rows differ, the shorter sequence determines the
number of valid pairs, and the excess rows are logged as unpaired and
excluded from the cleaned output.
...
\subsubsection{Justification of Simple Background Subtraction}

While advanced methods such as Asymmetric Least Squares (ALS), Stationary
Wavelet Transform (SWT), and Savitzky-Golay smoothing have been applied
to planetary LIBS data~\cite{Clegg2017,Anderson2022,Sridhar2025}, LunarAtlas
deliberately adopts simple paired subtraction for three reasons:
(i)~the L1 product already provides temporally matched background
acquisitions, making model-free subtraction physically grounded;
(ii)~reproducibility requires a deterministic, parameter-free operation---
ALS and wavelet methods introduce smoothing parameters that vary across
implementations; and (iii)~the infrastructure goal is to produce a
standardised cleaned tier, not a final denoised product---downstream users
can apply additional filtering as needed.
...
\subsection{Step 6: Metadata Association and Output Formatting}
...
\caption{Measurement~ID characteristics for the archival
  ...
  Peak intensity and its wavelength are reported for the cleaned
  spectra.  Wavelength channels: 2\,094 per ID.}
...
% =========================================================================
\section{LTTB+Peaks Adaptive Downsampling: Mathematical Formulation}
\label{sec:math}
% =========================================================================

\subsection{Problem Statement and Design Goals}

Let $S = \{(\lambda_i, I_i) : i = 0, \ldots, N-1\}$ be a cleaned
LIBS spectrum sorted by wavelength, with $N = 2094$ for Chandrayaan-3.
We seek a reduced representation $S'$ with $N' \ll N$ points such that:

\begin{enumerate}[nosep,leftmargin=*]
  \item \textbf{All local maxima are preserved.}  Every peak
    $i$ satisfying $I_i > I_{i-1}$ and $I_i > I_{i+1}$ appears in $S'$.
  \item \textbf{Global structure is retained.}  The visual envelope
    at each zoom level matches the full-resolution spectrum.
  \item \textbf{Zoom-saturation is mathematically defined.}  The
    system transitions to raw mode when further decimation would
    discard instrumentally resolved features.
  \item \textbf{Computation is fast.}  Response time remains below
    \SI{500}{\milli\second} for interactive use.
\end{enumerate}

The core innovation is the \textbf{union of LTTB-selected points with
all local maxima}, guaranteeing condition~(1) even when LTTB's
triangle-area heuristic misses narrow peaks.

\subsection{LTTB in Channel-Index Space}

Chandrayaan-3 LIBS has a \emph{fixed} 2\,094-channel grid.  We index
channels by integer $i \in [0, 2093]$ and define zoom level $k \in
\mathbb{Z}_{\geq 0}$, where $k=0$ is the widest view.  The target
number of downsampled points at zoom $k$ is:

\begin{equation}
\label{eq:nk}
N(k) = \min\!\bigl(2094,\; \lceil 2094 \cdot 2^{-k} \rceil\bigr).
\end{equation}

Table~\ref{tab:nk_zoom} shows representative values.

\begin{table}[htbp]
\centering
\caption{Target downsampled points $N(k)$ vs.\ zoom level $k$.}
\label{tab:nk_zoom}
{\small
\begin{tabularx}{\columnwidth}{@{}cccc@{}}
\toprule
$k$ & $N(k)$ & Reduction & Comment \\
\midrule
0 & 2\,094 & $1.0\times$ & Full resolution (no LTTB) \\
1 & 1\,047 & $2.0\times$ & Half decimation \\
2 & 524    & $4.0\times$ & Quarter decimation \\
3 & 262    & $8.0\times$ & Eighth decimation \\
4 & 131    & $16\times$  & Strong overview \\
\bottomrule
\end{tabularx}
}
\end{table}

The classical LTTB algorithm~\cite{Steinarsson2013} partitions the
index range $[0, N-1]$ into $N(k)$ buckets and selects one point per
bucket by maximising the triangle area formed with the previous
selected point and the next bucket's average.  For bucket $j$ with
indices $[i_{\text{start}}, i_{\text{end}}]$, let $P_a$ be the
previously selected point and $\bar{P}_b$ the average of bucket $j+1$.
For each candidate $P_c$ in bucket $j$, compute:
...
where $i$ denotes channel index and $I$ intensity.  The candidate with
$\max |A|$ is selected.  First and last points are always retained.

\begin{figure}[!t]
  \centering
  \includegraphics[width=0.95\columnwidth]{fig1_lttb_mechanism.png}
  \caption{\textbf{LTTB Triangle-Area Maximisation (Algorithmic
  Illustration).}  The high-resolution signal (grey circles) is
  partitioned into buckets (cyan shading).  For each bucket, LTTB
  selects the point (red star) that maximises the triangle area with
  the previous selected point (blue circle) and the next bucket's
  centroid (black cross).  This preserves the visual envelope while
  reducing point count.}
  \label{fig:lttb_mechanism}
\end{figure}

Figure~\ref{fig:lttb_mechanism} illustrates the triangle-area selection
heuristic.

\subsection{Peak-Union Guarantee}

LTTB is a \emph{greedy} algorithm that prioritises visual envelope
preservation but provides no guarantee of retaining local maxima.
For LIBS, where a 3-channel Ca~II doublet at 393--397~nm is
diagnostically critical, this is unacceptable.

We compute the set of all local maximum indices:

\begin{equation}
\label{eq:peaks}
P_{\text{peaks}} = \left\{ i \,\middle|\, I_i > I_{i-1} \;\wedge\; I_i > I_{i+1} \right\}.
\end{equation}

Let $P_{\text{LTTB}}$ be the set of indices selected by LTTB at zoom
level $k$.  The final downsampled index set is:
...
After sorting $P_{\text{final}}$, we extract the corresponding
$(\lambda_i, I_i)$ pairs to form $S'$.  This guarantees that
$|P_{\text{final}}| \geq |P_{\text{peaks}}|$, i.e., \textbf{all peaks
are retained regardless of bucket size}.

Figure~\ref{fig:perfect_comparison} demonstrates this guarantee on
real Chandrayaan-3 data.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig2_perfect_comparison.png}
  \caption{\textbf{LTTB+Peaks Perfect Structural Preservation.}
  (a)~Full-spectrum overlay: original 2\,094 points (grey) vs.\
  LTTB-selected 500 points (blue).  Visual fidelity is preserved
  across the entire wavelength range.  (b)~Detailed zoom (385--405~nm):
  the red path connects LTTB+peaks points, demonstrating that all
  local maxima (grey circles) are explicitly retained via the union
  step (Eq.~\ref{eq:union}).  No peaks are lost.}
  \label{fig:perfect_comparison}
\end{figure*}

\subsection{Dual Zoom-Saturation Criteria}

We define zoom saturation in terms of two independent constraints:
instrumental resolution and data density.

For a wavelength window $[\lambda_{\min}, \lambda_{\max}]$ with
$\Delta\lambda = \lambda_{\max} - \lambda_{\min}$, a baseline bucket
count $B_0 = 1000$ (approximately one bucket per screen pixel at
$k=0$), and minimum bucket width $b_{\min} = \SI{0.01}{\nano\metre}$
(approximately the instrumental resolution), the zoom-adjusted bucket
size is:

\begin{equation}
\label{eq:bsize}
b_{\text{size}}(z) = \frac{\Delta\lambda}{B_0 \cdot 2^z},
\end{equation}

where $z$ is the zoom index (same as $k$ for the full-range view).
The actual bucket width is clamped:

\begin{equation}
\label{eq:bfinal}
b_{\text{final}}(z) = \max\!\bigl(b_{\text{size}}(z),\, b_{\min}\bigr).
\end{equation}

The \emph{instrument-limited zoom index} is:
...
\left\lfloor
\log_2\!\left(
\frac{\Delta\lambda}{B_0 \, b_{\min}}
\right)
\right\rfloor.
\end{equation}

If the current window contains $n_{\text{pts}}$ raw samples, the
\emph{data-density-limited zoom index} is:
...
\right)
\right\rfloor
\quad \text{for } n_{\text{pts}} > B_0,
\end{equation}

and $z_{\text{data}} = z_{\text{instrument}}$ otherwise.

The dual-saturation zoom level is:

\begin{equation}
\label{eq:zmax}
z_{\max} = \min\!\bigl(z_{\text{instrument}},\, z_{\text{data}}\bigr).
\end{equation}

For $z \leq z_{\max}$, the system operates in \textbf{LTTB+peaks mode}.
For $z > z_{\max}$, it falls back to \textbf{raw mode}, returning all
$n_{\text{pts}}$ samples in the window.

Table~\ref{tab:zmax_windows} tabulates $z_{\max}$ for representative
windows.

\begin{table}[htbp]
\centering
\caption{Zoom saturation analysis for typical Chandrayaan-3 LIBS
  windows.  Parameters: $B_0 = 1000$, $b_{\min} = \SI{0.01}{\nano\metre}$.}
\label{tab:zmax_windows}
{\small
\begin{tabularx}{\columnwidth}{@{}Xccc@{}}
\toprule
Window & $\Delta\lambda$ (nm) & $z_{\max}$ & $b(z_{\max})$ (nm) \\
\midrule
Full band & 713.9 & 6 & 0.0112 \\
100~nm segment & 100.0 & 3 & 0.0125 \\
10~nm feature & 10.0 & 0 & 0.0100 \\
\bottomrule
\end{tabularx}
}
\end{table}

\subsection{Multi-Level Zoom Behaviour}

Figure~\ref{fig:adaptive_zoom} demonstrates the adaptive behaviour
across four zoom levels for a 250--450~nm segment.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig3_adaptive_zoom.png}
  \caption{\textbf{Adaptive Multi-Level Zoom with Automatic Saturation.}
  The same 250--450~nm segment rendered at four zoom levels:
  (a)~$z=0$ (full scan, 150 LTTB points);
  (b)~$z=1$ (segment zoom, 150 LTTB points);
  (c)~$z=2$ (feature zoom, 42 raw points);
  (d)~$z=3$ (raw resolution, 8 points).  The system automatically
  transitions from LTTB to raw mode at $z_{\max}=2$ for this window,
  ensuring no information loss at maximum magnification while
  maintaining fast rendering at coarser zoom levels.}
  \label{fig:adaptive_zoom}
\end{figure*}

At $z=0$ and $z=1$, LTTB+peaks provides strong decimation with peak
retention.  At $z=2$ and $z=3$, the window has crossed $z_{\max}$ and
raw mode is engaged, exposing full instrumental resolution.
...
\texttt{zoom\_level}, and \texttt{include\_nist}.  The response JSON
includes: metadata (observation ID, file MD5, algorithm version),
downsampling parameters ($N(k)$, $z_{\max}$, mode: LTTB or raw), and
an array of $(\lambda, I)$ pairs forming $S'$.  NIST reference lines
falling within the requested window are optionally overlaid.

\subsection{Progressive Loading Strategy}

The frontend follows a progressive loading protocol:
\begin{enumerate}[nosep,leftmargin=*]
  \item On initial load, request the full-range spectrum at $k = 0$
    for an overview ($N(0) = 2094$, typically rendered as
    $\sim$500 LTTB points).
  \item On each zoom or pan event, extract the new viewport
    $[\lambda_a, \lambda_b]$, compute $z_{\max}$ from
    Eq.~\eqref{eq:zmax}, and request the appropriate $k$.
  \item At $k > z_{\max}$, the backend returns raw samples,
    providing full spectral resolution inspection.
\end{enumerate}

This strategy ensures that the visualisation is always scientifically
faithful at every zoom level, with no client-side interpolation or
approximation.

\subsection{Additional Endpoints}

Supplementary endpoints expose: mission and instrument metadata;
measurement catalogs with per-measurement statistics; MD5 digests for
integrity verification; NIST reference lines for a requested
wavelength range; and cleaning-quality diagnostics (fraction of
negative values pre-clamping, baseline suppression factor).
...
times and pulse counts highlights the importance of the Measurement~ID
concept: lumping all four measurements into a single averaged spectrum
would mask this variability entirely.

Figure~\ref{fig:variability} visualises the four cleaned spectra
side-by-side, showing how ID~3 dominates with a high baseline while
ID~2 is nearly flat after background removal.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig5_variability.png}
  \caption{\textbf{Shot-to-Shot Variability in Cleaned Chandrayaan-3
  LIBS Spectra.}  Four panels show the cleaned spectra for
  Measurement~IDs~1--4 (a--d).  ID~3 (panel~c) exhibits a strong
  continuum baseline even after background subtraction, with
  prominent Mg~II emission near 280~nm and multiple Ca~II/Fe~I lines
  across 390--850~nm.  ID~2 (panel~b) shows near-complete background
  removal with 76.8\% negative channels clamped to zero, consistent
  with a ``null shot'' where detector noise dominated the signal.
  The Measurement~ID is essential for distinguishing these
  physically distinct plasma conditions.}
  \label{fig:variability}
\end{figure*}

\subsection{Downsampling Peak-Preservation Performance}

For the full 2\,094-channel Chandrayaan-3 spectrum at $k=2$
($N(2) = 524$ LTTB points), we validate peak retention:

\begin{itemize}[nosep,leftmargin=*]
  \item Raw spectrum: 2\,094 points, 87 local maxima (identified via
    Eq.~\eqref{eq:peaks}).
  \item LTTB-only (without union): 524 points, 63/87 peaks retained
    (72.4\% retention).
  \item LTTB+peaks (with union): 524 + 24 = 548 points, 87/87 peaks
    retained (\textbf{100\% retention}).
\end{itemize}

The 4.6\% increase in point count (524 → 548) buys absolute peak
fidelity, confirming the effectiveness of the union strategy.

\subsection{Emission Line Identification}
...
regolith~\cite{Sridhar2025,ISROCh3}.  Table~\ref{tab:emission_lines}
lists the most prominent LIBS emission lines and their NIST reference
wavelengths, matched against the cleaned spectra.

\begin{table}[htbp]
\centering
\caption{Prominent LIBS emission lines identified in cleaned
  Chandrayaan-3 spectra.  NIST reference wavelengths from the
  NIST Atomic Spectra Database~\cite{NIST}.}
\label{tab:emission_lines}
{\small
\begin{tabularx}{\columnwidth}{@{}lcX@{}}
\toprule
\textbf{Line} & \textbf{$\lambda_{\rm NIST}$~(nm)} &
  \textbf{Significance} \\
\midrule
Mg~II & 279.55 / 280.27 & Dominant doublet; major regolith
  constituent \\
...
detection, which identified aluminium, calcium, iron, chromium,
titanium, manganese, silicon, and oxygen from the Chandrayaan-3 LIBS
data~\cite{ISROCh3}.

\subsection{Performance Metrics}

On a modest server configuration (Intel Core i7, 16~GB RAM,
PostgreSQL 14 on SSD), end-to-end API response times (database query
+ LTTB+peaks downsampling + JSON serialisation) remain below
\SI{500}{\milli\second} for typical L1 spectra across all zoom levels.
Payload sizes range from approximately 10 to 400~kB under HTTP
compression depending on zoom level and spectral window.  The
composite index on \texttt{(measurement\_id, wavelength\_nm)}
provides approximately 15$\times$ speedup for spectral-range queries
...
\subsection{LTTB+Peaks as a Domain-Specific Downsampling Paradigm}

The union of LTTB-selected points with all local maxima
(Eq.~\ref{eq:union}) represents a \textbf{domain-specific adaptation}
of a generic time-series algorithm to the constraints of LIBS
spectroscopy.  Standard LTTB~\cite{Steinarsson2013} prioritises
visual fidelity but provides no semantic guarantees about feature
retention.  By explicitly injecting all peaks into the output, we
convert a heuristic into a \emph{deterministic guarantee}:
\textbf{no emission line is ever lost, regardless of bucket size or
zoom level}.

This paradigm is broadly applicable to any fixed-channel spectroscopic
instrument (XRF, Raman, FTIR, neutron activation) where narrow features
...
plasma/background separation, per-channel subtraction, MD5
integrity recording, and LTTB+peaks downsampling---are applicable to
any mission that delivers LIBS L1 products in a PDS4-compliant
wide-format CSV.  This includes future Indian lunar missions, proposed
asteroid LIBS instruments, and potentially Mars missions that release
raw L1 data in analogous formats~\cite{Saeidfirozeh2024}.
...
The LTTB+peaks algorithm has complexity $O(N(k) \times n_{\text{bucket}})$
where $n_{\text{bucket}} \approx N / N(k)$ is the average bucket size.
For $k=2$ ($N(2) = 524$ buckets over 2\,094 points),
$n_{\text{bucket}} \approx 4$, yielding $O(2000)$ triangle-area
comparisons---easily completed in $<$10~ms on modern hardware.
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item \textbf{Simple background pairing.}  Plasma and background
    shots are paired purely by temporal order.  More sophisticated
    pairing strategies---based on proximity in time, shared operation
    mode, or temperature matching---may improve cleaning quality
    for observations where the plasma/background interleaving
    is irregular.
...
PostgreSQL schema, an \textbf{LTTB+peaks adaptive downsampling
algorithm with mathematically defined dual zoom-saturation criteria
and a peak-union guarantee}, and a versioned API exposing
peak-preserving spectra with sub-\SI{500}{\milli\second}
response times.

The LTTB+peaks formulation converts a generic time-series heuristic
into a domain-specific guarantee: \textbf{no emission line is ever
lost, regardless of bucket size or zoom level}.  This paradigm is
broadly applicable to any fixed-channel spectroscopic instrument
where narrow features carry diagnostic information.
...
spectral validation.  We thank the open-source scientific Python
ecosystem (NumPy, SciPy, Pandas, Matplotlib, Astropy) and the
planetary science community whose LIBS instrument and calibration
...
design, system architecture, algorithm development (LTTB+peaks),
database design, API implementation, data analysis,

...
\fancyhead[R]{\small\textit{Preprint, April 2026}}
...
\noindent
{\footnotesize\textit{Planetary and Space Science} $\;\cdot\;$
Preprint submitted April 2026 $\;\cdot\;$
\textcopyright~2026 The Authors}
\par\vspace{8pt}\hrule\vspace{10pt}

{\centering
{\LARGE\bfseries LunarAtlas: Cleaning and Structuring\\[4pt]
Chandrayaan-3 LIBS Level-1 Data for\\[4pt]
Reproducible Lunar Spectroscopy}\par
\vspace{8pt}
...
min--max downsampling with a mathematically defined zoom-saturation
criterion, and a versioned API with sub-\SI{500}{\milli\second}
response times.
\end{abstract}
\vspace{4pt}
...
\begin{enumerate}[label=\textbf{(C\arabic*)},leftmargin=*,itemsep=3pt]
  ...
  \item \textbf{Zoom-aware visualisation.}  An adaptive min--max
    downsampling formulation---mathematically defined in terms of
    zoom level and wavelength span---that preserves emission-line peaks
    and baseline structure across all zoom levels, with an analytic
    zoom-saturation criterion identifying when the system must fall
    back to raw-data mode.
...
Section~\ref{sec:math} covers the adaptive downsampling formulation.
...
\cite{Sridhar2025}.  LunarAtlas complements this work by focusing on
the L1 data tier, providing a formal data model, reproducibility
infrastructure, and the Measurement~ID concept.
...
\caption{Column organisation of Chandrayaan-3 LIBS Level-1 CSV files.
  ...
  wavelength channels (headers are numeric wavelength values in nm);
  the final eight columns are instrument housekeeping parameters.}
\label{tab:l1_schema}
\begin{tabularx}{\columnwidth}{lX}
\toprule
\textbf{Column Group} & \textbf{Contents} \\
\midrule
Mission metadata (6) & Time, Measurement Count, Operation Mode,
  Measurement Type, Force Reset Status, Laser Fire Status \\
\midrule
Wavelength channels (2\,049) & Column names are wavelength values
  in nm (164.35, 164.74, \ldots, 878.26); cell values are intensity
  in raw counts \\
\midrule
Instrument housekeeping (8) & Delay Time ($\mu$s), Integration Time
  ($\mu$s), Number of Pulses, X-Factor, Laser Energy (V), Laser Pump
  Current (A), PRR (Hz), On/Off Status \\
\bottomrule
\end{tabularx}
\end{table}
...
The system comprises three tiers (Fig.~\ref{fig:architecture}):
...
\texttt{observation} & id, instrument\_id, start\_time, stop\_time, site \\
\texttt{file\_version} & id, observation\_id, filename, filesize, md5, ingested\_at, algorithm\_version \\
\texttt{measurement} & id, file\_version\_id, measurement\_id\_in\_file, time\_utc, measurement\_type, laser\_energy\_v, integration\_time\_us, n\_pulses, prr\_hz \\
\texttt{spectral\_data} & id, measurement\_id (FK), wavelength\_nm, cleaned\_intensity, is\_valid\_plasma, is\_background \\
\texttt{nist\_reference\_lines} & id, element, ionisation, wavelength\_nm, aki\_value, ei\_ev, ek\_ev \\
\bottomrule
\end{tabularx}
\end{table}
...
per wavelength channel.  The resulting long-form schema is:

\begin{equation}
\label{eq:long_form}
\underbrace{\texttt{Meas\_ID},\ \texttt{Time\_UTC},\ \texttt{Meas\_Count}}_{
  \text{measurement keys}}
,\
\underbrace{\texttt{Wavelength\_nm},\ \texttt{Raw\_Intensity}}_{
  \text{spectral data}}
,\
\underbrace{\texttt{Delay},\ \texttt{Integration},\ \ldots}_{
  \text{housekeeping}}
\end{equation}
...
\label{eq:bg_mask}
\text{Background:}\quad&
  \texttt{Force\_Reset\_Status} = 1
  \;\wedge\;
  \texttt{Laser\_Fire\_Status} = 0 \\
\label{eq:plasma_mask}
\text{Plasma:}\quad&
  \texttt{Force\_Reset\_Status} = 0
  \;\wedge\;
...
above 50\% may indicate a mismatched plasma--background pair.
...
\begin{table}[htbp]
\centering
\caption{Columns in the cleaned output CSV.  The first three columns
  identify the measurement; \texttt{Wavelength\_nm} and
  \texttt{Cleaned\_Intensity} are the primary spectral data; columns
  6--11 are mission flag fields; columns 12--19 are instrument
...
\midrule
\texttt{Measurement\_ID} & Integer label (1, 2, 3, \ldots) assigned
  sequentially to each plasma--background pair within a file \\
\texttt{Time\_UTC} & UTC acquisition timestamp from the plasma row \\
\texttt{Measurement\_Count} & Instrument-level counter from the plasma
  row \\
\texttt{Wavelength\_nm} & Wavelength in nm (derived from column header) \\
\texttt{Cleaned\_Intensity} & Background-subtracted, non-negative
  intensity in counts \\
\texttt{Operation\_Mode} & PDS4 operation mode string (e.g.,
  \texttt{ASLF}) \\
\texttt{Measurement\_Type} & Measurement type string (e.g.,
  \texttt{EP}) \\
\texttt{Force\_Reset\_Status} & 0 for normal (plasma) operation \\
\texttt{Laser\_Fire\_Status} & 1 for laser-fired acquisition \\
\texttt{Is\_Valid\_Plasma} & Boolean: \texttt{True} for plasma
  measurements \\
\texttt{Is\_Background} & Boolean: \texttt{False} for plasma
  measurements \\
\texttt{Delay\_Time\_us} & Delay between laser pulse and detection
  gate (\SI{}{\micro\second}) \\
\texttt{Integration\_Time\_us} & Detector integration window
  (\SI{}{\micro\second}) \\
\texttt{Number\_of\_Pulses} & Laser pulses per acquisition \\
\texttt{X\_Factor} & Internal gain scaling factor \\
\texttt{Laser\_Energy\_V} & Laser energy setting (V) \\
\texttt{Laser\_Pump\_Current\_A} & Laser pump diode current (A) \\
\texttt{PRR\_Hz} & Pulse repetition rate (Hz) \\
\texttt{On\_Off\_Status} & Instrument power status flag \\
\bottomrule
\end{tabularx}
\end{table}
...
Fig.~\ref{fig:all_overlaid} overlays all four cleaned spectra on a
single axis, demonstrating the dynamic range of intensities across
measurements and the locations of known emission lines.
Fig.~\ref{fig:individual_subplots} shows each Measurement~ID in its
own panel, with instrument metadata annotated directly on the plot.
Fig.~\ref{fig:heatmap} displays the spectral heatmap
(Measurement~$\times$ Wavelength), where the colour scale encodes
cleaned intensity; the bright vertical stripe near 282--283~nm
corresponds to the Mg~II emission cluster visible in IDs~3 and~4.
Fig.~\ref{fig:peak_comparison} compares peak intensities and laser
energies across IDs.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{ch3_lib_002_20230825T104221_00_l1_cleaned_02_individual_subplots.png}
...
spectrum sorted by wavelength, with $\lambda_{\min} \leq \lambda_i
\leq \lambda_{\max}$.  Define $\Delta\lambda = \lambda_{\max} -
\lambda_{\min}$.  We wish to construct a reduced representation
$S'$ with $N' \ll N$ data points such that:
\begin{enumerate}[nosep,leftmargin=*]
  \item all NIST-matched emission-line peaks are represented in $S'$;
  \item baseline structure is preserved via minimum-intensity points;
  \item the bucket size adapts to the user's zoom level;
  \item computation and serialisation are fast enough for
    sub-\SI{500}{\milli\second} API response.
\end{enumerate}

\subsection{Zoom-Dependent Bucket Size}

For zoom level $z \in \mathbb{Z}_{\geq 0}$, the unconstrained
bucket size is:

\begin{equation}
\label{eq:bsize}
b_{\text{size}}(z) =
  \frac{\Delta\lambda}{\text{BASE\_BUCKETS} \times 2^z},
\end{equation}

where $\text{BASE\_BUCKETS} \approx 1000$ gives approximately one
bucket per screen pixel at $z = 0$.  To prevent buckets smaller than
the instrument's spectral resolution, a minimum bucket size
$b_{\min}$ (e.g., \SI{0.01}{\nano\metre}) is enforced:
...
$N_b = \lceil \Delta\lambda \,/\, b_{\text{final}} \rceil$.

\subsection{Zoom Saturation and the Raw-Data Fallback}

The \emph{zoom saturation} threshold is the maximum zoom level at
which $b_{\text{size}}(z) \geq b_{\min}$:
...
For $z > z_{\max}$, bucket size is clamped at $b_{\min}$ and the
backend returns raw spectral samples in the requested window.
Table~\ref{tab:zmax} tabulates $z_{\max}$ for representative windows.

\begin{table}[htbp]
\centering
\caption{Zoom saturation analysis.  Parameters:
  $\text{BASE\_BUCKETS} = 1000$, $b_{\min} = \SI{0.01}{\nano\metre}$.}
\label{tab:zmax}
\begin{tabularx}{\columnwidth}{X c c c}
\toprule
\textbf{Window} & $\Delta\lambda$~(nm) & $z_{\max}$ & $b(z_{\max})$~(nm) \\
\midrule
...
\subsection{Min--Max Aggregation with Overlapping Boundaries}

To prevent narrow peaks from being split across two adjacent buckets,
each bucket $j$ is extended by an overlap fraction
$o_{\text{pct}} = 0.05$ (5\%):

\begin{equation}
\lambda_{\text{start}}^{\text{ext}}(j) =
  \begin{cases}
    \lambda_{\text{start}}(j) - o_{\text{pct}}\,b_{\text{final}} & j > 0 \\
    \lambda_{\min} & j = 0
  \end{cases}
\end{equation}

For all wavelength samples $(\lambda_i, I_i)$ falling within
bucket~$j$'s extended boundaries, the bucket records:

\begin{align}
I_{\min}(j) &= \min_{i \in \text{bucket}\,j} I_i, &
\lambda_{\min}(j) &= \arg\min I_i, \\
I_{\max}(j) &= \max_{i \in \text{bucket}\,j} I_i, &
\lambda_{\max}(j) &= \arg\max I_i.
\end{align}

This representation retains both the baseline (via $I_{\min}$) and
the peak (via $I_{\max}$) of each bucket.  Rendering the two as a
vertical range bar faithfully depicts the spectral fine structure
at each zoom level.

\subsection{NIST-Guided Peak Insertion}

After bucket aggregation, any NIST reference line whose nearest
wavelength sample falls within a bucket but whose peak intensity is
not the $I_{\max}$ of that bucket---due to bucket averaging or
mis-alignment---is explicitly inserted into the output as a
single-point record.  This guarantees 100\% retention of reference
lines regardless of bucket size.
...
bucket parameters ($b_{\text{final}}$, $N_b$, $z_{\max}$), and an
array of bucket objects.  Each bucket object contains
$\lambda_{\min}(j)$, $I_{\min}(j)$, $\lambda_{\max}(j)$,
$I_{\max}(j)$, $N_{\text{points}}(j)$, and optional NIST annotations.

\subsection{Progressive Loading Strategy}

The frontend follows a progressive loading protocol:
\begin{enumerate}[nosep,leftmargin=*]
  \item On initial load, request the full-range spectrum at $z = 0$
    for an overview.
  \item On each zoom or pan event, extract the new viewport
    $[\lambda_a, \lambda_b]$, compute $z_{\max}(\lambda_b -
    \lambda_a)$ from Eq.~\eqref{eq:zmax}, and request the
    appropriate zoom level.
  \item At $z > z_{\max}$, the backend returns raw samples,
...
\caption{Cleaned intensity statistics per Measurement~ID for the
  full spectral range (164.35--878.26~nm).}
\label{tab:measurement_stats}
\begin{tabularx}{\columnwidth}{c c c c c}
\toprule
\textbf{ID} & \textbf{Mean~(cts)} & \textbf{Std~(cts)} &
  \textbf{Max~(cts)} & \textbf{$E$~(V)} \\
\midrule
1 & 61.0  & 56.9  & 321  & 3327 \\
2 & 9.4   & 23.8  & 261  & 3280 \\
3 & 416.9 & 95.0  & 684  & 3194 \\
4 & 54.1  & 62.4  & 637  & 3202 \\
\bottomrule
\end{tabularx}
\end{table}

The striking difference between ID~3 (mean~417~counts) and ID~2
(mean~9~counts) despite similar integration times and pulse counts
...
emission is concentrated near 283~nm (Mg~II doublet).
...
+ aggregation + JSON serialisation) remain below \SI{500}{\milli
\second} for typical L1 spectra across all zoom levels.

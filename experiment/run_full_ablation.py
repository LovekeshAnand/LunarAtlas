...
def run_ablation():
    ...
    for idx, filepath in enumerate(selected_files):
        ...
        for pair_id in range(n_pairs):
            ...
            h_target = np.isin(nist_indices, hybrid_indices).sum() / n_nist
...
\\begin{{itemize}}
    ...
    \\item \\textbf{{Standard LTTB (1.0\\% and 10.0\\%):}} The Largest-Triangle-Three-Buckets algorithm, which selects points that maximize the triangular area between adjacent buckets.

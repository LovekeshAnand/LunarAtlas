...
CREATE TABLE IF NOT EXISTS spectral_data_p2 PARTITION OF spectral_data FOR VALUES FROM (2000) TO (3000);
CREATE TABLE IF NOT EXISTS spectral_data_p3 PARTITION OF spectral_data FOR VALUES FROM (3000) TO (4000);
CREATE TABLE IF NOT EXISTS spectral_data_p4 PARTITION OF spectral_data FOR VALUES FROM (4000) TO (5000);
CREATE INDEX IF NOT EXISTS idx_wavelength_brin_p0 ON spectral_data_p0 USING BRIN (wavelength_nm) WITH (pages_per_range = 64);
CREATE INDEX IF NOT EXISTS idx_wavelength_brin_p1 ON spectral_data_p1 USING BRIN (wavelength_nm) WITH (pages_per_range = 64);
CREATE INDEX IF NOT EXISTS idx_wavelength_brin_p2 ON spectral_data_p2 USING BRIN (wavelength_nm) WITH (pages_per_range = 64);
CREATE INDEX IF NOT EXISTS idx_wavelength_brin_p3 ON spectral_data_p3 USING BRIN (wavelength_nm) WITH (pages_per_range = 64);
CREATE INDEX IF NOT EXISTS idx_wavelength_brin_p4 ON spectral_data_p4 USING BRIN (wavelength_nm) WITH (pages_per_range = 64);

-- NIST reference line indexes
CREATE TABLE IF NOT EXISTS nist_lines (id SERIAL PRIMARY KEY, element VARCHAR(10) NOT NULL, ionization_stage VARCHAR(10), wavelength_nm NUMERIC(10,4) NOT NULL, aki NUMERIC, ei_ev NUMERIC, ek_ev NUMERIC, lower_level VARCHAR(255), upper_level VARCHAR(255), relative_intensity INTEGER, created_at TIMESTAMPTZ DEFAULT NOW());
CREATE INDEX IF NOT EXISTS idx_nist_wavelength ON nist_lines(wavelength_nm);

-- Sample Data
INSERT INTO mission (name, agency) VALUES ('Chandrayaan-3', 'ISRO') ON CONFLICT DO NOTHING;
INSERT INTO instrument (mission_id, name) VALUES (1, 'LIBS') ON CONFLICT DO NOTHING;
INSERT INTO observation (instrument_id, start_time, target_name) VALUES (1, NOW(), 'Moon') ON CONFLICT DO NOTHING;
INSERT INTO file_version (observation_id, filename) VALUES (1, 'test.dat') ON CONFLICT DO NOTHING;
INSERT INTO measurement (file_version_id, measurement_id, time_utc) VALUES (1, 1, NOW()), (1, 2, NOW()) ON CONFLICT DO NOTHING;
EOF
